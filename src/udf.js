...
class UDF {
    ...
    loadSymbols() {
        ...
        const promise = this.exchange.exchangeInfo().catch(err => {
            console.error(err)
            setTimeout(() => {
                this.loadSymbols()
            }, 1000)
        ...
        this.symbols = promise.then(info => {
            return info.symbols.map(symbol => {
                return {
                    ...
                    listed_exchange: this.exchange.constructor.name,
                    type: 'crypto',
    ...
    async history(symbol, from, to, resolution) {
        ...
        while (true) {
            const klines = await this.exchange.klines(symbol, interval, from, to, 500)
