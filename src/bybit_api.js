...
module.exports = class Binance {
     ...
     */
    exchangeInfo() {
        return this.request('/api.bybit.com/v2/public/symbols')
    }
    ...
    klines(symbol, interval, startTime, endTime, limit) {
        return this.request('/api.bybit.com/v2/public/kline/list', { qs: { symbol, interval, startTime, endTime, limit } })
    }
    ...
    request(path, options) {
        return new Promise((resolve, reject) => {
            request('https://api.bybit.com' + path, options, (err, res, body) => {
                if (err) {
                    return reject(err)
                }
                if (!body) {
